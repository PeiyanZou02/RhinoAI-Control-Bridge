...
namespace RhinoAI
{
    public static class ProductExport
    {
        ...
        public static string Save(Snapshot snapshot,Raster raster,Config config,string dir,Dictionary<string,string> files,out string placementConstraint)
        {
            ...
            if(snapshot.Wallpaper==null)throw new InvalidOperationException("No Wallpaper was captured.");
            ...
            placementConstraint=string.Format(System.Globalization.CultureInfo.InvariantCulture,"MANDATORY REFERENCE APPEARANCE LOCK: reference is the sole authority for the complete frame's color or monochrome mode, white balance, exposure, brightness, contrast, tonal range, colors and background. Match reference exactly in every area outside the object edit region. Never average, blend or transfer color, darkness, grey tint, exposure or contrast from scale_lock, placement, masks, depth, normals, edges, shape_lock or material_id. Those images are technical data only. MANDATORY CLEAN FINAL OUTPUT: return a natural finished photograph only. Never copy any technical overlay from scale_lock into the result: no magenta/pink/purple outline, bounding rectangle, center crosshair, guide line, marker, diagram, label or measurement graphic. Treat every overlay pixel in scale_lock as invisible metadata, not as part of the object or scene. Reconstruct the clean photograph behind every guide mark using reference and placement. FULL-FRAME PLACEMENT LOCK: the full placement and scale_lock images are the absolute authority for final object position, pixel size, rotation and silhouette. On the {0}x{1} full canvas, copy the object bounding box exactly: left={2}px, top={3}px, right={4}px, bottom={5}px; normalized box=({6:F6},{7:F6})-({8:F6},{9:F6}); normalized center=({10:F6},{11:F6}); normalized width={12:F6}, height={13:F6}; exact silhouette area={14} pixels ({15:F6} of the full canvas). The final object silhouette must touch the same left, right, top and bottom extrema shown in scale_lock and must not extend beyond that box. Technical detail images are approximately {16:F2}x magnified digital crops for surface detail only. Never use their apparent object size, never enlarge the object, and never move it outside this full-frame placement box. Before returning the image, perform a final cleanup check: restore the reference image's original appearance outside the edit region and remove every technical overlay while preserving the object itself.",w,h,x0,y0,x1+1,y1+1,nx0,ny0,nx1,ny1,(nx0+nx1)*0.5,(ny0+ny1)*0.5,nx1-nx0,ny1-ny0,indices.Length,indices.Length/(double)(w*h),zoom);
            ...
            {
                if(protect.Width!=w||protect.Height!=h)throw new InvalidOperationException("The occlusion mask must match the export size "+w+" × "+h+". White protects, black does not.");
            ...
            {
                // Preserve the reference tones: darkening this technical image can leak a grey cast
                // into multimodal generation when the model visually averages its inputs.
                using(var g=Graphics.FromImage(scaleLock))g.DrawImageUnscaled(background,0,0);
            ...
            string prompt="\nBACKGROUND BLEND\n"+config.WearInstructions+"\nUse reference.png as the aligned base photograph. placement.png indicates the exact projected object placement; its flat materials are only a guide. object_mask isolates the object silhouette; inpaint_mask white means editable and black means preserve. Keep pixels outside the edit mask unchanged using masked compositing in the workflow. Depth, normal and edge maps describe only the exported object geometry, not the photographed scene. Do not infer whole-scene depth from the object-only depth image. Respect the visible object outline and design. Handle natural occlusion by foreground elements and subtle contact shadows. Return a clean photograph with no magenta outline, rectangle, crosshair, guide line, marker or other technical overlay from scale_lock.\n";
            File.WriteAllText(Path.Combine(dir,"blend_prompt.txt"),prompt,Encoding.UTF8);
            File.WriteAllText(Path.Combine(dir,"blend.json"),JsonConvert.SerializeObject(new{schema="rhino-ai-blend/1",width=w,height=h,reference="reference.png",referenceAlignment="Rhino native viewport background capture at export dimensions; no guessed crop",placement="placement.png",scaleLock="scale_lock.png",placementDetail="placement_detail.png",detailCropPixels=new[]{detailRect.Left,detailRect.Top,detailRect.Right,detailRect.Bottom},detailCamera=detailCamera,detailProjection="Exact sub-frustum/digital crop of the original camera; identical camera location, orientation, near plane, lens and perspective rays; no dolly, lens change or re-framing",objectMask="mask.png",editMask="inpaint_mask.png",requestedMaskPadding=config.MaskPadding,effectiveMaskPadding=effectivePadding,automaticEditRegion=config.AutoEditRegion,maskConvention="white=edit; black=preserve; use ImageToMask red channel, NOT LoadImage alpha MASK output",occlusionMask=string.IsNullOrWhiteSpace(config.OcclusionMask)?null:"occlusion_mask.png",boundsPixels=new[]{x0,y0,x1+1,y1+1},boundsNormalized=new[]{x0/(double)w,y0/(double)h,(x1+1)/(double)w,(y1+1)/(double)h},camera=snapshot.Camera,prompt=prompt},Formatting.Indented),Encoding.UTF8);
            return prompt;
        }
