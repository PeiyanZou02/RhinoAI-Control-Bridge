...
[assembly: System.Reflection.AssemblyVersion("0.20.0.0")]
...
namespace RhinoAI
{
    ...
    public sealed class BridgeWindow : Form
    {
        ...
        public BridgeWindow(RhinoDoc document)
        {
            ...
            sceneAspect.Text="普通场景固定旧版宽屏比例（1024 × 589）";sceneAspect.AutoSize=true;sceneAspect.Checked=config.LockSceneAspect;
            ...
            Row(form,"ComfyUI 地址",server);Row(form,"导出目录",Browse(output,false,true));Row(form,"API 工作流（可选）",Browse(workflow,true,false));Row(form,"输出最长边",edge);Row(form,"普通场景画幅",sceneAspect);Row(form,"对象范围",selected);Row(form,"自动同步",autoSync);
        ...
        void Read()
        {
            ...
            config.Server=server.Text.Trim();config.Output=output.Text.Trim();config.Workflow=workflow.Text.Trim();config.LongEdge=(int)edge.Value;config.LockSceneAspect=sceneAspect.Checked;config.SelectedOnly=selected.Checked;config.ProductMode=tryon.Checked;config.MatchWallpaperAspect=wallpaperAspect.Checked;config.DetailPriority=detailPriority.Checked;config.AutoEditRegion=adaptiveMask.Checked;config.Product=product.Text;config.WearInstructions=wear.Text;config.MaskPadding=(int)padding.Value;config.OcclusionMask=occlusion.Text.Trim();config.AutoSync=autoSync.Checked;
        }
        ...
        {
            return string.Join("|",new[]{server.Text,output.Text,workflow.Text,edge.Value.ToString(),sceneAspect.Checked.ToString(),selected.Checked.ToString(),tryon.Checked.ToString(),wallpaperAspect.Checked.ToString(),detailPriority.Checked.ToString(),adaptiveMask.Checked.ToString(),product.Text,wear.Text,padding.Value.ToString(),occlusion.Text,AutoSyncWatcher.FileStamp(occlusion.Text),string.Join(";",layers.Rows.Cast<DataGridViewRow>().Select(r=>string.Join("|",r.Cells.Cast<DataGridViewCell>().Select(c=>Convert.ToString(c.Value)))))});
        }
