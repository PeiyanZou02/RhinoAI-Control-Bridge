...
namespace RhinoAI
{
    ...
    public sealed class Config
    {
        ...
        public string Product="ring / 戒指";
        public string WearInstructions="Place the ring naturally around the intended finger at the exact projected location, scale and orientation of the supplied product. Preserve the original face, skin texture, anatomy, pose, clothing and background. Let the finger occlude the rear band where physically appropriate. Preserve the jewelry design, prongs, gemstone count and proportions. Add subtle contact shadows and realistic metal/gem reflections consistent with the photograph.";
        public int MaskPadding=12;
        public string OcclusionMask="";
        public int LongEdge=1024;
        public string Style="Photorealistic visualization, physically plausible materials, soft natural lighting, accurate scale, balanced exposure, fine surface detail. Preserve the supplied product or scene design.";
    ...
    public static class MaterialRules
    {
        ...
        {
            string name=layer.FullPath.ToLowerInvariant();string mat="待指定 / neutral matte material",desc="Neutral warm-grey matte finish, fine uniform texture, low specular reflection; replace this suggestion with the intended material.";
            if(Has(name,"ceramic","porcelain","陶瓷","瓷")){mat="釉面陶瓷 / glazed ceramic";desc="Warm ivory glazed ceramic, smooth continuous glaze with subtle handmade surface variation, soft broad highlights, roughness 0.15–0.30; preserve the original sculpted form, edge thickness and openings.";}
            else if(Has(name,"gold","黄金","金色","18k")){mat="18K 黄金 / 18k yellow gold";desc="Polished 18k yellow gold, warm rich gold tone, metallic reflectance, roughness 0.12–0.22, finely controlled highlights and subtle micro-scratches at realistic jewelry scale; preserve prongs, edges and engraving.";}
            else if(Has(name,"diamond","gem","钻石","宝石")){mat="切割宝石 / faceted gemstone";desc="Clear faceted gemstone with crisp facet boundaries, physically plausible refraction and restrained spectral dispersion, high optical clarity; preserve the exact stone count, cut, size and setting. Do not add extra gemstones.";}
            else if(Has(name,"silver","银","platinum","铂")){mat="抛光银色金属 / polished silver metal";desc="Cool polished silver-colored precious metal, high metallic reflectance, roughness 0.10–0.22, realistic studio reflections, subtle micro-scratches and crisp jewelry edges; no plastic appearance.";}
            else if(Has(name,"glass","glazing","玻璃")){mat="透明玻璃 / clear glass";desc="Clear low-iron glass, subtle edge tint, smooth surface, roughness 0.03–0.10, physically plausible transparency and Fresnel reflections; preserve the exact boundaries and thickness.";}
            else if(Has(name,"wood","timber","木")){mat="天然橡木 / natural oak";desc="Natural light oak, warm beige-brown tone, fine longitudinal grain at realistic scale, satin clear finish, roughness 0.40–0.55; align grain with each panel's long direction.";}
            else if(Has(name,"metal","alum","steel","金属","铝","钢","窗框","frame")){mat="银色金属 / satin silver metal";desc="Neutral silver satin metal, fine uniform surface grain, metallic reflectance, roughness 0.25–0.40, balanced highlights and crisp edges; preserve the source shape and scale. Adjust this suggested finish to match the intended metal.";}
            else if(Has(name,"concrete","混凝土","水泥","墙","wall")){mat="米白微水泥 / off-white microcement";desc="Warm off-white microcement, seamless fine mineral texture, faint tonal variation, matte roughness 0.70–0.85, diffuse reflection, no oversized cracks or repetitive texture.";}
            else if(Has(name,"stone","marble","石","大理石")){mat="浅色石材 / light limestone";desc="Warm ivory honed limestone, subtle natural mineral variation and sparse fine pores, roughness 0.50–0.70, restrained reflectivity, realistic slab scale and carefully aligned joints.";}
            else if(Has(name,"plant","grass","tree","绿植","草","树")){mat="自然植被 / vegetation";desc="Natural muted green vegetation with subtle hue variation, fine leaf detail and soft light transmission, realistic leaf scale, no artificial plastic shine.";}
            else if(Has(name,"fabric","cloth","布","软包")){mat="亚麻织物 / linen fabric";desc="Warm neutral woven linen, fine visible weave at real scale, matte roughness 0.85–0.95, soft diffuse reflection, gentle fabric variation.";}
    ...
    public static class Exporter
    {
        ...
        {
            var view=doc.Views.ActiveView;if(view==null)throw new InvalidOperationException("请先激活一个模型视口。");
            if(view is RhinoPageView)throw new InvalidOperationException("请切换到模型空间视口后导出。");
            ...
            foreach(var obj in objects)
            {
                ...
                if(clip!=null && clip.ViewportIds().Contains(vp.Id))throw new InvalidOperationException("当前视口启用了剖切平面。此版本尚不支持剖切，请先禁用该视口的剖切再导出，以避免生成错误控制图。");
            ...
            if(s.Triangles.Count==0)throw new InvalidOperationException("没有可导出的表面。支持 Brep、Extrusion、Mesh、SubD 和块实例。");
        ...
        {
            if(nesting>32)throw new InvalidOperationException("块实例嵌套超过 32 层。");
            ...
            if(meshes.Count==0){s.Warnings.Add("对象无法生成网格："+obj.Id);return;}
        ...
        public static ExportResult Render(Snapshot snapshot,Config config,Action<int> progress)
        {
            ...
            var html=new StringBuilder("<!doctype html><meta charset='utf-8'><title>Rhino AI · Color Code</title><style>body{font:16px system-ui;background:#16191e;color:#eee;margin:32px}img{max-width:100%;max-height:65vh}table{border-collapse:collapse;width:100%}td,th{text-align:left;padding:12px;border-bottom:1px solid #444}pre{white-space:pre-wrap;line-height:1.6}</style><h1>Rhino AI · Color Code</h1><img src='color_code.png'><table><tr><th>编码颜色</th><th>Rhino 图层</th><th>目标材质</th></tr>");
            ...
            if(config.ProductMode)html.Append("<h2>产品佩戴 · 对齐参考 / 位置 / 编辑蒙版</h2><img src='reference.png'><img src='placement.png'><img src='inpaint_mask.png'><pre>"+Escape(wear)+"</pre>");
            html.Append("<h2>导出说明</h2><pre>"+Escape(string.Join("\n",snapshot.Warnings))+"</pre>");
        ...
        static byte[] CaptureRendered(RhinoView view,Guid viewport,int width,int height,HashSet<Guid> selected)
        {
            ...
            try
            {
                ...
                {if(bitmap==null)throw new InvalidOperationException("Rendered 视图捕获失败。");bitmap.Save(memory,ImageFormat.Png);return memory.ToArray();}
            }
