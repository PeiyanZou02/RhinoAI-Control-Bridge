...
const detailGuide=imageGuide({...detailData,prompts:{placement_constraint:'FULL-FRAME PLACEMENT LOCK: normalized box=(0.48,0.52)-(0.53,0.60).'}},channelsFor(detailData));assert(detailGuide.includes('IDENTICAL camera projection'));assert(detailGuide.includes('Never infer a new camera'));assert(detailGuide.includes('ABSOLUTE FULL-FRAME authority'));assert(detailGuide.includes('normalized box='));
assert(detailGuide.includes('EDITABLE NEIGHBORHOOD'));assert(detailGuide.includes('white area is never a scale'));
assert(detailGuide.includes('FULL-FRAME PIXEL SCALE stencil'));assert(detailGuide.includes('same left/right/top/bottom bounds'));
await updateGraph(graph,all,node,()=>{});assert(batchConnected(graph,unfilled,all));
assert.equal(unfilled.inputs.filter(i=>i.link!=null).length,9);
