...
namespace RhinoAI
{
    public static class IntegrationTests
    {
        ...
        public static string Run(RhinoDoc doc,string root)
        {
            ...
            var fixedScene=Exporter.WallpaperLayout(900,900,1024,589,1024);Assert(fixedScene.OutputWidth==1024&&fixedScene.OutputHeight==589&&fixedScene.CaptureWidth==1024&&fixedScene.CaptureHeight==1024&&fixedScene.Crop.Top>0,"scene layout remains 1024x589 when the Rhino viewport becomes square");
            var fixedSceneCamera=ProductExport.CropCamera(new Camera{Width=fixedScene.CaptureWidth,Height=fixedScene.CaptureHeight,Left=-1,Right=1,Bottom=-1,Top=1,Near=1,Far=100,Perspective=true},fixedScene.Crop,fixedScene.OutputWidth,fixedScene.OutputHeight);Assert(Math.Abs((fixedSceneCamera.Right-fixedSceneCamera.Left)/(fixedSceneCamera.Top-fixedSceneCamera.Bottom)-1024.0/589)<0.002,"fixed scene crop preserves one shared perspective sub-frustum");
            var files=r.Save(Path.Combine(root,"synthetic"),new Dictionary<int,int>{{1,0xff0000}});Assert(files.Count==13&&files.ContainsKey("shape_lock")&&files.ContainsKey("material_mask_1"),"twelve aligned raster channels including shape lock, plus one mask per material");
            using(var map=new Bitmap(files["color_code"]))Assert((map.GetPixel(32,32).ToArgb()&0xffffff)==0xff0000,"color code exact RGB");
            using(var map=new Bitmap(files["material_id"]))Assert((map.GetPixel(32,32).ToArgb()&0xffffff)==0xff0000,"material ID uses the exact layer color published in the material prompt");
