...
badge.textContent="Rhino: loading sync extension…";
...
async function diagnostic(state,message) {
  try {await fetch(new URL("api/userdata/rhino_ai_frontend_status.json?overwrite=true",base),{
    ...
    body:JSON.stringify({schema:"rhino-ai-frontend/1",version:23,state,message})
  });}catch{}
}
await diagnostic("bootstrap","Rhino frontend module entry point reached");
try {await import("./main.mjs?v=23");}
catch(error) {
  badge.textContent="Rhino extension failed to load: "+error.message;badge.style.borderColor="#e56767";
