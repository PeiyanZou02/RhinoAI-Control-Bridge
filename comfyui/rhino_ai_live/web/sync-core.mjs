...
const ROLES = {
  reference:"MANDATORY PRIMARY BASE IMAGE and final canvas; edit directly into this photograph while preserving its subject identity, anatomy, pose, camera, composition, clothing, background and all unedited pixels. Never replace the scene or output the product by itself",
...
export function imageGuide(data, channels=channelsFor(data)) {
  ...
  if(channels.includes("reference"))lines.push("The reference image is the mandatory base canvas for the final output. Return an edited version of that same scene, keep its framing and background, and integrate the product at the placement shown; never return an isolated product on a new background.");
